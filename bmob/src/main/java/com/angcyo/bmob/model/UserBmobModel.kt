...
import com.angcyo.bmob.bean.UserBmob
import com.angcyo.bmob.bean.showName
...
open class UserBmobModel : ViewModel() {
    ...
    companion object {
        ...
        val allUserInfo = mutableListOf<UserBmob>()

        /**拉取所有用户*/
        fun fetchAllUser() {
            val query = BmobQuery<UserBmob>()
            query.findObjects(object : FindListener<UserBmob>() {
                override fun done(results: MutableList<UserBmob>?, error: BmobException?) {
                    results?.run {
                        this.forEach {
                            updateUserInfo(it)
                        }
        ...
        /**更新内存缓存用户的信息*/
        fun updateUserInfo(info: UserBmob) {
            var find = false
        ...
        /**返回已经存在的用户信息, 并更新*/
        fun fetchAndGetUserBmob(userId: Long): UserBmob? {
            var result: UserBmob? = null
    ...
    //登录的用户信息
    val userBmob = MutableLiveData<UserBmob>()
    ...
    /**登录成功*/
    open fun onLoginSucceed(user: UserBmob) {
